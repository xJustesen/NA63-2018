...
% 
% angles_x = load(strcat(datpath, 'angles_mean_x_46.txt'));
% [~,idx] = sort(angles_x(:,1));
% angles_x = angles_x(idx, :);
% angx = angles_x(:,1); angxc = angles_x(:,2);
% 
% angles_y = load(strcat(datpath, 'angles_mean_y_46.txt'));
...
% ylabel('$\langle d\phi_x \rangle$ [rad]','fontsize',22,'interpreter','latex')
% xlim([-200e-6, 600e-6]);
% box on
% set(gca, 'FontSize', 18)
% subplot(2,1,2)
...
% ylabel('$\langle d\phi_y \rangle$ [rad]','fontsize',22,'interpreter','latex')
% xlim([-250e-6, 250e-6]);
% box on
% set(gca, 'FontSize', 18)
% set(f, 'Units','centimeters','PaperUnits','centimeters', 'PaperSize',[18, 12],'PaperPosition',[0, 0, 18, 12],'Position',[0 0 18 12])
% print('../../presentation/figures/80GeV_1.5mm_axis_run46.svg', '-dsvg','-r600','-painters')
