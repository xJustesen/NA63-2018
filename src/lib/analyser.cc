...
analyser::analyser(vector<double> z, const char *name, string run, string beamparams)
{
    ...
    data_path_ = "/home/christian/Documents/cern2018/data";
...
#pragma omp atomic
    ...
    string eventfile_name = "/home/christian/Documents/cern2018/spectre/cuts/events_run_" + run_number_ + ".txt";
...
#pragma omp critical
                ...
                for (size_t j = 0; j < tracks.size(); j++)
                {
                    ...
                    {
                        double M3_distance = CalculateDistance(tracks[j][2][0], tracks[j][2][1], tracks[k][2][0], tracks[k][2][1]);
                        if (M3_distance < foil_paired_tracks_lim_)
                        {
                            photons_within_cut++;
...
bool analyser::SortVectorDescending(const vector<double> &p1, const vector<double> &p2)
{
    return p1[1] > p2[1];
}
