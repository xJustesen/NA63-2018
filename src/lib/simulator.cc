...
void simulator::MimosaDetector(int detector, int event, int &detections, mt19937_64 generator, vector<vector<double>> local_particles)
{
    ...
    for (size_t i = 0; i < local_particles.size(); i++)
    {
        ...
        if (inside_bounds)
        {
            ...
            if (hotpix == -1)
            {
                ...
                if (mimosas_[detector][event].size() > 0)
                {
                    double dist = CalculateDistance(local_particles[i][0] + Dx, local_particles[i][1] + Dy, mimosas_[detector][event][0][0], mimosas_[detector][event][0][1]);
                    ...
                    {
                        mimosas_[detector][event].push_back({local_particles[i][0] + Dx, local_particles[i][1] + Dy});
                    }
                    else
                    {
                        ...
                        double dx = mimosas_[detector][event][0][0] - local_particles[i][0] - Dx;
                        double dy = mimosas_[detector][event][0][1] - local_particles[i][1] - Dy;
                        mimosas_[detector][event][0][0] += dx / 2.0;
                        mimosas_[detector][event][0][1] += dy / 2.0;
                    }
                }
                else
                {
    ...
    if ((double)rand() / RAND_MAX < fake_hit_probability)
    {
        ...
        mimosas_[detector][event].push_back(hit);
    }
}
